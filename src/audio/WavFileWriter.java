...
public class WavFileWriter 
{
    ...
    private float calc(int i, float changeRate, double[] overtoneStrengths) {
    	double total = 0;
    	for (double overtoneStrength : overtoneStrengths) total += overtoneStrength;
    	double result = 0;
    	for (int j = 0; j < overtoneStrengths.length; j++) result += Math.sin(i * changeRate * (j+1)) * overtoneStrengths[j] / total;
    	return (float) result;
    }
